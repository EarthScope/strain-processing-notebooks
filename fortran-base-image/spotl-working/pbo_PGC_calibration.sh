#!/bin/sh
#
#  Script that deal with PGC sites only  
#
...
#
   bin="/Applications/spotl/bin"
   work="/Applications/spotl/working"
   here=`pwd`;
   cd $work

#  Calculate the M2 ocean load for the ocean model but exclude the area in poly.$6

   $bin/nloadf $1 $2 $3 $4   $work/m2.$5      $work/green.$7.std     l   $work/poly.gefu   - > $here/ex1m2.f1 

#  Calculate the M2 ocean lead for the regional model ($6) but only for the region in poly.$6

   $bin/nloadf $1 $2 $3 $4   $work/m2.gefu    $work/green.$7.std  l   $work/poly.gefumod  > $here/ex1m2.f2 
   $bin/nloadf $1 $2 $3 $4   $work/m2.pgc     $work/green.$7.std   l   $work/poly.pgc      > $here/ex1m2.f3 
   echo "nloadf $1 $2 $3 $4   m2.pgc    green.$7.std   l   poly.pgc      > ex1m2.f3 "

#  Add the M2 loads computed above together

   cat $here/ex1m2.f1   $here/ex1m2.f2 | $bin/loadcomb c >  $here/ex1m2.tmp
   cat $here/ex1m2.tmp  $here/ex1m2.f3 | $bin/loadcomb c >  $here/tide.m2

#  Calculate the O1 ocean load for the ocean model but exclude the area in poly.$6

   $bin/nloadf $1 $2 $3 $4   $work/o1.$5      $work/green.$7.std    l   $work/poly.gefu   - > $here/ex1o1.f1 

#  Calculate the O1 ocean lead for the regional model ($6) but only for the region in poly.$6

   $bin/nloadf $1 $2 $3 $4   $work/o1.gefu    $work/green.$7.std   l   $work/poly.gefumod > $here/ex1o1.f2 
   $bin/nloadf $1 $2 $3 $4   $work/o1.pgc     $work/green.$7.std  l   $work/poly.pgc     > $here/ex1o1.f3 

#  Add the O1 loads computed above together

   cat $here/ex1o1.f1   $here/ex1o1.f2 | $bin/loadcomb c >  $here/ex1o1.tmp
   cat $here/ex1o1.tmp  $here/ex1o1.f3 | $bin/loadcomb c >  $here/tide.o1
...
#  Combine ocean load and solid earth tides
   ...
   echo "cat  tide.o1 | loadcomb t  >>  $1.tide.total"
...
#  Convert back to amplitude (nanostrain ) and phase (degrees)
   ...
   echo "pbo_tide_PGC         	$1	$2	$3	$4	m2.$5	green.$7.std	poly.$6 \n " > $here/$1.theory.txt
...
#   rm $here/ex1m2.f1 $here/ex1m2.f2 $here/ex1m2.f3 $here/ex1o1.f1 $here/ex1o1.f2 $here/ex1o1.f3 $here/tide.o1  $here/tide.m2
